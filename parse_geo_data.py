...
for geo_id in gse_ids:
    ...
    std_scaler.fit_transform(merged_std_std)
    merged_std_std = pd.DataFrame(std_scaler.fit_transform(merged_std_std), columns=merged_std_std.columns, index=merged_std_std.index)

    mtx['GB_ACC'] = gb_acc
    mtx = mtx.dropna()
    mtx['GB_ACC'] = [i.split('.')[0] for i in mtx['GB_ACC'].values]
    mtx['median'] = mtx.iloc[:,:-1].median(axis=0).values
    ...
    mtx.to_csv('./data/normal/{}_n11n.txt.gz'.format(geo_id), index=True, compression='gzip', sep='\t')
...
conc_data.to_csv('./data/normal/GEOfull_no_n11n.txt.gz', index=True, compression='gzip', sep='\t')

conc_data = conc_data[~conc_data.index.duplicated(keep=False)]
batch = conc_data.loc['batch']
conc_data = conc_data.drop(index='batch')

# normalize
rank_mean = conc_data.astype('float64').stack().groupby(conc_data.astype('float64').rank(method='first').stack().astype(int)).mean()
conc_data = conc_data.astype('float64').rank(method='min').stack().astype(int).map(rank_mean).unstack()

# leave only 12000 most expressed genes
conc_data['mean'] = conc_data.mean(axis=0).values
conc_data = conc_data.sort_values(by=['mean'], ascending=False)
conc_data = conc_data.head(n=15000).drop('mean', axis=1)

conc_data = conc_data.append(batch)

conc_data.to_csv('./data/normal/GEO12000g_n11n.txt.gz', index=True, compression='gzip', sep='\t')
